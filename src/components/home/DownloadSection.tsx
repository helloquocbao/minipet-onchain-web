...
export const DownloadSection = () => {
  ...
  return (
    <section
      id="download"
      className="pb-10 md:pb-20 bg-transparent relative overflow-hidden"
    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto mb-16">
          {downloads.map((d, idx) => (
            ...
            >
              ...
              <div
                className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl ${d.color
                  .replace(/dark:[^\s]+ /g, "")
                  .replace("bg-", "bg-")
                  .replace(
                    "/10",
                    "/5",
                  )} border border-slate-100 flex items-center justify-center mb-4 sm:mb-5 transition-transform group-hover:scale-110 duration-300 shadow-sm`}
              >
                {d.icon}
              </div>

              <h3 className="text-[15px] sm:text-[17px] font-black text-slate-900 mb-1 sm:mb-2">
                {d.platform}
              </h3>
              {d.note && (
                <span className="inline-block px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-green-50 text-green-600 border border-green-200 rounded-full mb-2">
                  {d.note}
                </span>
              )}
              <p className="text-[12px] sm:text-[13px] text-slate-500 mb-4 sm:mb-5 max-w-[200px] leading-relaxed">
                {d.desc}
              </p>
              ...
              <div className="w-full pt-6 border-t border-slate-100">
                <a
                  href={d.link}
                  aria-label={`Download MiniPet for ${d.platform} (${d.ext})`}
                  className="group/btn relative inline-flex items-center justify-center gap-2 w-full py-3 sm:py-3.5 rounded-full text-[12px] sm:text-[13px] font-bold no-underline transition-all duration-300 active:scale-[0.98] bg-indigo-600 hover:bg-indigo-700 text-white shadow-md shadow-indigo-500/20"
                >
                  <Download
                    size={16}
                    className="group-hover/btn:translate-y-0.5 transition-transform duration-300"
                  />
                  <span>{`${t("download.btn")} ${d.ext}`}</span>
                </a>
              ...
              <div className="mt-6 pl-14 border-t border-slate-100 pt-6">
                ...
                <div className="space-y-4 mb-8">
                  <div className="flex items-start gap-3.5 text-[13px] font-bold text-slate-700">
                    <div className="w-6 h-6 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center text-[11px] flex-shrink-0 font-mono font-bold border border-indigo-100">
                      1
                    </div>
                    <span className="mt-0.5">
                      {t("download.troubleshooting.macStep2")}
                    </span>
                  </div>
                  <div className="flex items-start gap-3.5 text-[13px] font-bold text-slate-700">
                    <div className="w-6 h-6 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center text-[11px] flex-shrink-0 font-mono font-bold border border-indigo-100">
                    ...
                    <span className="mt-0.5">
                      {t("download.troubleshooting.macStep3")}
                    </span>
                  </div>
                </div>
                  ...
                  <div className="bg-slate-900 rounded-2xl p-5 shadow-xl relative overflow-hidden group">
                    ...
                    <div className="font-mono text-[13px] leading-relaxed">
                      <div className="text-slate-500 mb-2">
                        #{" "}
                        {
                          t("download.troubleshooting.macTerminalStep").split(
                            ":",
                          )[0]
                        }
                        :
                      ...
                      <div className="flex items-start gap-2.5">
                        ...
                        <code className="text-indigo-100 break-all select-all hover:text-white transition-colors cursor-pointer">
                          {t("download.troubleshooting.macTerminalStep")
                            .split(":")[1]
                            ?.trim() ||
                            t("download.troubleshooting.macTerminalStep")}
                        </code>
